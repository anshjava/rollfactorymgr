...
public class RollEditView implements FxmlView<RollEditViewModel>, Initializable {
    ...
    void onApplyChanges(ActionEvent actionEvent) {
        Roll editedRoll = Roll.builder()
                ...
                .value(getValueByRollType(rollType.getValue()))
                .build();
    ...
    private void refreshView() {
        id.textProperty().set(getRollProperty().getId().get().toString());
        sku.textProperty().set(getRollProperty().getSku().get());
        rollType.setValue(RollType.byTypeName(getRollProperty().getRollType().get()));
        paper.setValue(Paper.byWeight(getRollProperty().getPaperWeight().get()));
        widthType.setValue(WidthType.byWidth(getRollProperty().getRollWidth().get()));
        coreType.setValue(CoreType.byDiameter(getRollProperty().getCoreDiameter().get()));
        rollLength.textProperty().set(getRollProperty().getRollLength().get().toString());
        rollDiameter.textProperty().set(getRollProperty().getRollDiameter().get().toString());
    ...
    public void reCalculateAll() {
        Roll rollToCalculate = Roll.builder()
                .id(Long.parseLong(id.getText()))
                .sku(sku.getText())
                .rollType(rollType.getValue())
                .paper(paper.getValue())
                ...
                .value(getValueByRollType(rollType.getValue()))
                .build();
        rollWeight.textProperty().set(viewModel.calculateWeight(rollToCalculate).toString());
        if (rollType.getValue() == RollType.LENGTH) {
            rollDiameter.textProperty().set(viewModel.calculateDiameter(rollToCalculate).toString());
        } else {
            rollLength.textProperty().set(viewModel.calculateLength(rollToCalculate).toString());
        }
    }
}
